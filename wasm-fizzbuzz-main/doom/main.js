...
// 초고화질을 위해 명암 단계를 더 정밀하게 표현하는 촘촘한 문자 세트
const asciiChars = ["#", "W", "M", "B", "R", "$", "@", "%", "*", "o", "a", "+", ";", "=", "-", ":", ",", ".", " "];

function drawCanvas(ptr) {
    const width = 640;
    const height = 400;
    const maxOffset = width * height * 4;
    var doom_screen = new Uint8ClampedArray(memory.buffer, ptr, maxOffset);
    const output = document.getElementById("output");
    
    // 🚀 성능 최적화: 수만 개의 글자를 더할 때 렉을 방지하기 위해 배열(Array) 방식을 사용합니다.
    let asciiRows = [];

    // 건너뛰기 없이(x++, y++) 모든 픽셀을 1:1로 전부 읽습니다!
    for (let y = 0; y < height; y++) {
        let row = "";
        for (let x = 0; x < width; x++) {
            let offset = (y * width + x) * 4;
            if (offset >= maxOffset - 4) continue;

            let r = doom_screen[offset];
            let g = doom_screen[offset + 1];
            let b = doom_screen[offset + 2];
    ...
    // 배열을 하나로 합쳐서 한 번에 화면에 주입 (렉 감소 핵심)
    output.innerText = asciiRows.join("\n");
    ++number_of_draws;
}
